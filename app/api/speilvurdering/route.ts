...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const forsteDagForYtelse: string | null = body?.["førsteDagForYtelse"] ?? null
        const ytelse: string | null = body?.ytelse ?? null
        ...
        if (forsteDagForYtelse !== null && !datoRegex.test(forsteDagForYtelse)) {
            return new NextResponse(JSON.stringify({ error: "Ugyldig førsteDagForYtelse" }), { status: 400 })
        }
        ...
        const backendBody = {
            fnr: fnr,
            "førsteDagForYtelse": forsteDagForYtelse,
            periode: {
                fom: periode.fom,
                tom: periode.tom,
            },
        ...
        const backendUrl = `${SYKEPENGER_API_BASE_URL}/speilvurdering`
